...
async function check(viewport,name,touch){
  ...
  let ready=0;
  page.on('console',m=>{messages.push(m.text());if(m.text().includes('GATE3_READY'))ready++;if(m.type()==='error'||/SCRIPT ERROR:|^ERROR:/.test(m.text()))errors.push(m.text());});
  ...
  try {
    ...
    await until(()=>ready===1,'Gate 3 ready',60000);
    ...
    await canvas.screenshot({path:`build/validation/interaction-${name}-start.png`});
    await walk(120);
    await use();
    ...
    const popup=await canvas.screenshot({path:`build/validation/interaction-${name}-memory.png`});
    ...
    await canvas.screenshot({path:`build/validation/interaction-${name}-complete.png`});
    const next=ready+1;
    ...
    console.log(`INTERACTION_WEB_PASSED: ${name} sword pickup, combat, memory, popup and restart`);
  }finally{
    await writeFile(`build/validation/interaction-${name}.log`,[...messages,...errors].join('\n'));
    await page.screenshot({path:`build/validation/interaction-${name}-final.png`}).catch(()=>{});
    await context.close();
  }
}
